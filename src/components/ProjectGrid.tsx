...
interface ProjectGridProps {
  projects: Project[];
  color?: string; // Optional: Pass 'cyan', 'purple', etc. to match the section
}

const ProjectGrid: React.FC<ProjectGridProps> = ({ projects, color = "cyan" }) => {
      ...
      {projects.map((proj) => (
        ...
        >
          ...
          <div className="relative z-10 flex items-center gap-5 pt-4 border-t border-zinc-800 group-hover:border-white/10 transition-colors mt-auto">
            ...
            {/* YouTube Icon - Always Shows */}
            <a 
              href={proj.links.youtube || '#'} // Uses '#' if link is missing
              target="_blank" 
